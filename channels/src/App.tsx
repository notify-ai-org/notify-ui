...
import { Field, Provider, providers, runtime } from './schema';
import './styles.css';
  ...
  return <div className="app-shell channels-portal">
    ...
    <main className="main-content">
      <div className="page-heading"><div><div className="eyebrow">DELIVERY CONTROL</div><h1>Channels<span>.</span></h1><p>Connect your providers. Keep every delivery under control.</p></div>
        <button className="primary" disabled={busy} onClick={()=>setCreating('EMAIL')}><Plus size={16}/> New channel</button></div>
      ...
      : <div className="channel-workspace">
        ...
        {selected && provider && <section className="channel-detail panel">
          ...
          <div className="detail-body">
            ...
            {tab==='secrets' && <>
              <div className="section-heading"><div><h3>{selected.secretRef?'Replace credentials':'Connect your provider'}</h3><p>Stored values are never displayed. Enter the complete credential set.</p></div><KeyRound className="accent" size={23}/></div>
              ...
              <form key={`${selected.id}-${revision}`} autoComplete="off" onSubmit={e=>{e.preventDefault();const form=e.currentTarget;const data=new FormData(form);const credentials=Object.fromEntries(provider.secrets.map(f=>[f.key,String(data.get(f.key)??'')]).filter(([,v])=>v.trim()));form.reset();void dispatch(mutateChannel({kind:'credentials',id:selected.id,replace:!!selected.secretRef,credentials},selected.secretRef?'Credentials replaced.':'Credentials saved and channel enabled.'));}}><fieldset disabled={busy||detailsLoading||!!detailsError||!!metadata?.status&&metadata.status!=='ACTIVE'}><div className="field-grid">{provider.secrets.map(f=><FieldInput key={f.key} field={f} secret/>)}</div><div className="form-footer"><span>Values are cleared after submission.</span><button className="primary" disabled={busy}>{busy?'Saving…':selected.secretRef?'Replace credentials':'Save credentials & enable'}</button></div></fieldset></form>
