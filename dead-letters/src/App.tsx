...
const STATUS_COLOR: Record<DLQStatus, string> = {
  PENDING: '#f59e0b', REPLAYED: '#22c55e', DISCARDED: '#ef4444',
};
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ width: 600 }}>
        <div className="modal-header">
          <span className="modal-title">Payload — <span className="mono" style={{ color: '#ef4444', fontSize: 13 }}>{entry.notificationId}</span></span>
...
function DLQTable() {
  ...
  const isRetrying = (id: number) => (retrying as number[]).includes(id);

  const pending = (items as DeadLetterEntry[]).filter(e => e.replayStatus === 'PENDING').length;
  const discarded = (items as DeadLetterEntry[]).filter(e => e.replayStatus === 'DISCARDED').length;
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {(['ALL', 'PENDING', 'REPLAYED', 'DISCARDED'] as const).map(s => (
        ...
        <div style={{ overflowX: 'auto' }}>
          {loading ? <div style={{ padding: 24 }}>{[...Array(5)].map((_, i) => <div key={i} className="skeleton" style={{ height: 44, marginBottom: 8, borderRadius: 6 }} />)}</div> : (
            <table className="data-table">
              <thead><tr><th>Notification</th><th>Channel</th><th>Target</th><th>Failure</th><th>Attempts</th><th>Status</th><th>Created</th><th>Last Attempt</th><th>Actions</th></tr></thead>
              <tbody>
                {(items as DeadLetterEntry[]).map((e) => (
                  <tr key={e.id}>
                    <td><span className="mono" style={{ color: '#ef4444' }}>{e.notificationId}</span></td>
                    <td><span className="mono" style={{ color: '#94a3b8', fontSize: 11 }}>{e.channel}</span></td>
                    <td><span style={{ color: '#94a3b8', fontSize: 12 }}>{e.target || '—'}</span></td>
                    <td style={{ maxWidth: 200 }}>
                      <span style={{ color: '#f87171', fontSize: 12, display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        [{e.failureReasonCode}] {e.failureMessage ?? e.failureCategory}
                      </span>
                    </td>
                    <td style={{ textAlign: 'center' }}>
                      <span style={{ color: '#94a3b8' }}>{e.attemptCount}</span>
                    </td>
                    <td><span className="badge" style={{ background: `${STATUS_COLOR[e.replayStatus]}22`, color: STATUS_COLOR[e.replayStatus] }}>{e.replayStatus}</span></td>
                    <td style={{ color: '#475569' }}>{formatTimestamp(e.createdAt)}</td>
                    <td style={{ color: '#334155' }}>{formatTimestamp(e.lastAttemptAt)}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button className="btn-icon" title="View payload" onClick={() => setViewEntry(e)}><Eye size={13} /></button>
                        <button className="btn-icon" title="Replay" disabled={isRetrying(e.id) || e.replayStatus !== 'PENDING'}
                        ...
                        <button className="btn-icon" title="Discard" disabled={e.replayStatus !== 'PENDING'} style={{ color: e.replayStatus !== 'PENDING' ? '#334155' : '#ef4444' }}
                ...
                {!items.length && <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: '#475569' }}>Dead letter queue is empty.</td></tr>}
              </tbody>
            </table>
          )}
        </div>
