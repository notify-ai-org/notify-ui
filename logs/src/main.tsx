...
function LogTable({ kind, rows, loading }: { kind: LogKind; rows: Array<Record<string, unknown>>; loading: boolean }) {
  ...
  const columns = useMemo<LogColumn[]>(() => {
    ...
    const hidden = new Set(kind === 'capture'
      ? ['payload', 'headers', 'correlationId', 'processingStatus', 'tenantId']
      : ['payload', 'headers']);
    return rows.length ? Object.keys(rows[0]).filter(key => !hidden.has(key)).slice(0, 8).map(key => ({ key, label: formatColumnLabel(key) })) : [];
  }, [kind, rows]);
  return <><div style={{ overflowX: 'auto' }}><table className="data-table"><thead><tr>{columns.map(column => <th key={column.key}>{column.label}</th>)}<th>Details</th></tr></thead><tbody>{rows.map((row, index) => <tr key={String(row.id ?? index)}>{columns.map(column => <td key={column.key}>{displayValue(row[column.key])}</td>)}<td><button className="btn-icon" title="View event details" onClick={() => setSelected(row)}><Eye size={13} /></button></td></tr>)}{!loading && !rows.length && <tr><td colSpan={Math.max(columns.length + 1, 1)} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>No log records found.</td></tr>}</tbody></table></div>{selected && <LogDetailsModal row={selected} onClose={() => setSelected(null)} />}</>;
}
...
function LogDetailsModal({ row, onClose }: { row: Record<string, unknown>; onClose: () => void }) {
  return <div className="modal-overlay" onClick={onClose}><div className="modal-box" style={{ maxWidth: 760 }} onClick={event => event.stopPropagation()}><div className="modal-header"><span className="modal-title">Event details</span><button className="btn-icon" onClick={onClose}><X size={15} /></button></div><div className="modal-body" style={{ display: 'grid', gap: 12, maxHeight: '70vh', overflow: 'auto' }}>{Object.entries(row).map(([key, value]) => <DetailBlock key={key} label={formatColumnLabel(key)} value={value} />)}</div><div className="modal-footer"><button className="btn btn-primary" onClick={onClose}>Done</button></div></div></div>;
}
function DetailBlock({ label, value }: { label: string; value: unknown }) {
  return <div><div className="form-label">{label}</div>{isPlainObject(value) ? <div style={{ border: '1px solid var(--border)', padding: 12, background: 'rgba(255,255,255,0.02)', display: 'grid', gap: 8 }}>{Object.entries(value).map(([key, child]) => <DetailBlock key={key} label={key} value={child} />)}</div> : Array.isArray(value) ? <div style={{ display: 'grid', gap: 8 }}>{value.length ? value.map((item, index) => <DetailBlock key={index} label={`Item ${index + 1}`} value={item} />) : <span style={{ color: 'var(--text-muted)' }}>Empty</span>}</div> : <div style={{ color: '#d0c9a8', overflowWrap: 'anywhere', whiteSpace: 'pre-wrap' }}>{displayValue(value)}</div>}</div>;
}
function isPlainObject(value: unknown): value is Record<string, unknown> { return !!value && typeof value === 'object' && !Array.isArray(value); }
function formatColumnLabel(key: string) { return key.replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/_/g, ' ').replace(/\b\w/g, char => char.toUpperCase()).replace(/\bId\b/g, 'ID'); }
